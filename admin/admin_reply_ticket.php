...
$time_worked = hesk_getTime($_POST['time_worked']);
...
$sql = "UPDATE `".hesk_dbEscape($hesk_settings['db_pfix'])."tickets` SET `status`='$new_status',`lastreplier`='1',`replierid`='".hesk_dbEscape($_SESSION['id'])."',`time_worked` = ADDTIME(`time_worked`,'" . hesk_dbEscape($time_worked) . "') ";
...
elseif ($_SESSION['afterreply'] == 2)
{
    ...
    $sql .= ' `owner` IN (0,'.hesk_dbEscape($_SESSION['id']).') AND ';
	$sql .= hesk_myCategories();
    $sql .= ' AND `status` IN (\'0\',\'1\') ';
    $sql .= ' ORDER BY `owner` DESC, `priority` ASC LIMIT 1';
