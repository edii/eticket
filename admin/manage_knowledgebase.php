...
if (!isset($_SESSION['hide']['treemenu']))
{
	...
	<p>
    <img src="../img/view.png" width="16" height="16" border="0" alt="" style="padding:1px" class="optionWhiteNbOFF" /> <a href="manage_knowledgebase.php?a=list_private"><?php echo $hesklang['listp']; ?></a><br >
    <img src="../img/view.png" width="16" height="16" border="0" alt="" style="padding:1px" class="optionWhiteNbOFF" /> <a href="manage_knowledgebase.php?a=list_draft"><?php echo $hesklang['listd']; ?></a><br />
    <img src="../img/link.png" width="16" height="16" border="0" alt="" style="padding:1px" class="optionWhiteNbOFF" /> <a href="http://support.mozilla.com/en-US/kb/how-to-write-knowledge-base-articles" rel="nofollow" target="_blank"><?php echo $hesklang['goodkb']; ?></a></p>
...
function list_draft() {
    ...
    <?php
    $sql = "SELECT * FROM `". hesk_dbEscape($hesk_settings['db_pfix'])."kb_articles` WHERE `type`='2' GROUP BY `catid` ORDER BY `catid` ASC, `id` ASC";
	$res = hesk_dbQuery($sql);
    $num = hesk_dbNumRows($res);

    if ($num == 0)
    {
    	echo '<p>'.$hesklang['kb_no_dart'].' &nbsp; <a href="manage_knowledgebase.php?a=add_article&amp;catid='.$catid.'"><img src="../img/add_article.png" width="16" height="16" alt="'.$hesklang['kb_i_art2'].'" title="'.$hesklang['kb_i_art2'].'" border="0" style="border:none;vertical-align:text-bottom" /></a> <a href="manage_knowledgebase.php?a=add_article&amp;catid='.$catid.'"><b>'.$hesklang['kb_i_art2'].'</b></a></p>';
    }
    else
    {
    	?>
...
function list_private() {
    ...
    $sql = "SELECT `id` FROM `". hesk_dbEscape($hesk_settings['db_pfix'])."kb_categories` WHERE `type`='1'";
	$res = hesk_dbQuery($sql);
    $num = hesk_dbNumRows($res);
    if ($num)
    {
    ...
    <?php
    $sql = "SELECT * FROM `". hesk_dbEscape($hesk_settings['db_pfix'])."kb_articles` WHERE `type`='1' ";
	if (count($private_categories))
    {
		$sql.= " OR `catid` IN(" . implode(',', $private_categories) . ") ";
    }
    $sql.= " GROUP BY `catid` ORDER BY `catid` ASC, `id` ASC";
	$res = hesk_dbQuery($sql);
    $num = hesk_dbNumRows($res);

    if ($num == 0)
    {
    	echo '<p>'.$hesklang['kb_no_part'].' &nbsp; <a href="manage_knowledgebase.php?a=add_article&amp;catid='.$catid.'"><img src="../img/add_article.png" width="16" height="16" alt="'.$hesklang['kb_i_art2'].'" title="'.$hesklang['kb_i_art2'].'" border="0" style="border:none;vertical-align:text-bottom" /></a> <a href="manage_knowledgebase.php?a=add_article&amp;catid='.$catid.'"><b>'.$hesklang['kb_i_art2'].'</b></a></p>';
    }
    else
    {
    	?>
...
function import_article() {
    ...
    /* Get ticket ID */
    $trackingID = hesk_cleanID( hesk_REQUEST('track') );
	if (empty($trackingID))
	{
		return false;
	}
...
function remove_kb_att() {
	...
	$sql = 'UPDATE `'.hesk_dbEscape($hesk_settings['db_pfix']).'kb_articles` SET `attachments`=\''.hesk_dbEscape($art['attachments']).'\', `history`=CONCAT(`history`,\''.hesk_dbEscape($revision).'\') WHERE `id`='.hesk_dbEscape($id).' LIMIT 1';
	hesk_dbQuery($sql);
...
function save_article() {
    ...
    $keywords = hesk_input($_POST['keywords']);
...
function manage_category() {
    ...
    <?php
    $sql = 'SELECT * FROM `'.hesk_dbEscape($hesk_settings['db_pfix']).'kb_articles` WHERE `catid`='.hesk_dbEscape($catid).' ORDER BY `sticky` DESC, `art_order` ASC';
	$result = hesk_dbQuery($sql);
    ...
    else
    {
	    ...
	    $sql2 = "SELECT COUNT(*) FROM `".hesk_dbEscape($hesk_settings['db_pfix'])."kb_articles` WHERE `catid`=".hesk_dbEscape($catid)." AND `sticky` = '1' ";
...
function new_article() {
	...
	/* Get the latest reply_order */
	$sql = "SELECT `art_order` FROM `".hesk_dbEscape($hesk_settings['db_pfix'])."kb_articles` WHERE `catid`=".hesk_dbEscape($catid)." AND `sticky` = '" . hesk_dbEscape($sticky) . "' ORDER BY `art_order` DESC LIMIT 1";
	$res = hesk_dbQuery($sql);
	$row = hesk_dbFetchRow($res);
	$my_order = $row[0]+10;

    /* Insert article into database */
	$sql = "INSERT INTO `".hesk_dbEscape($hesk_settings['db_pfix'])."kb_articles` (`catid`,`dt`,`author`,`subject`,`content`,`keywords`,`type`,`html`,`sticky`,`art_order`,`history`,`attachments`) VALUES (
    '".hesk_dbEscape($catid)."',
    NOW(),
...
function show_treeMenu() {
	...
	?>
	<script src="<?php echo HESK_PATH; ?>inc/treemenu/TreeMenu.js" language="JavaScript" type="text/javascript"></script>
...
function toggle_sticky()
{
	...
	$sql = "UPDATE `".hesk_dbEscape($hesk_settings['db_pfix'])."kb_articles` SET `sticky`='" . hesk_dbEscape($sticky) . " ' WHERE `id`=" . hesk_dbEscape($id) . " LIMIT 1";
	$result = hesk_dbQuery($sql);
...
function update_article_order($catid)
{
	...
	$sql = 'SELECT `id`, `sticky` FROM `'.hesk_dbEscape($hesk_settings['db_pfix']).'kb_articles` WHERE `catid`='.hesk_dbEscape($catid).' ORDER BY `sticky` DESC, `art_order` ASC';
	$res = hesk_dbQuery($sql);
