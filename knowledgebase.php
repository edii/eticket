...
function hesk_show_kb_article($artid) {
    ...
    {
	    $sql = 'UPDATE `'.hesk_dbEscape($hesk_settings['db_pfix']).'kb_articles` SET `views`=`views`+1 WHERE `id`=\''.hesk_dbEscape($artid).'\' LIMIT 1';
		hesk_dbQuery($sql);
    }
...
<tr>
	...
	<td>
		...
		while ($cat = hesk_dbFetchAssoc($result))
		{
		    ...
		    {
		    	$sql = 'SELECT `id`,`subject` FROM `'.hesk_dbEscape($hesk_settings['db_pfix']).'kb_articles` WHERE `catid`=\''.hesk_dbEscape($cat['id']).'\' AND `type`=\'0\' ORDER BY `sticky` DESC, `views` DESC, `art_order` ASC LIMIT '.hesk_dbEscape($hesk_settings['kb_numshow']+1);
		        $res = hesk_dbQuery($sql);
		        $num = 1;
...
<tr>
	...
	<?php
	$sql = 'SELECT * FROM `'.hesk_dbEscape($hesk_settings['db_pfix']).'kb_articles` WHERE `catid`=\''.hesk_dbEscape($catid).'\' AND `type`=\'0\' ORDER BY `sticky` DESC, `art_order` ASC';
	$res = hesk_dbQuery($sql);
